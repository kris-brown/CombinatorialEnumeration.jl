...
srch_leftinv = c->[crels_to_cset(leftinv, r) for r in search(leftinv, c)]
...
srch_equalizer = c->[crels_to_cset(equalizer, r)
...
srch_perm = c -> [crels_to_cset(perm, r) for r in search(perm, [c])]
...
srch_reflg = cs -> [crels_to_cset(reflg, r) for r in search(reflg, cs)]

for (consts, expected) in [
    [1,1]=>1,   # Single node with self loop
...
srch_const2 = cs -> [crels_to_cset(const2, r) for r in search(const2, cs)]
...
srch_inflist = cs -> [crels_to_cset(inflist, r) for r in search(inflist, cs)]
srch_inflist([1,0,0])
...
srch_trips = cs -> [crels_to_cset(trips, r) for r in search(trips, cs)]
...
srch_sg = cs -> [crels_to_cset(semig, r) for r in search(semig, cs)]
